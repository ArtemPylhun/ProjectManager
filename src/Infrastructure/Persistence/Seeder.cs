    ...
    public static class Seeder
    {
        ...
        public static async Task SeedProjects(this IApplicationBuilder app)
        {
            ...
            if (!context.Projects.Any())
            {
                ...
                var projects = new[]
                {
                    Project.New(ProjectId.New(), "Website Development", "Building a new company website", DateTime.UtcNow, adminUser.Id, "#73aaee", usualUser.Id),
                    Project.New(ProjectId.New(), "Mobile App Upgrade", "Upgrading the mobile application", DateTime.UtcNow.AddDays(1), adminUser.Id, "#b8d7fb", usualUser.Id)
                };
        ...
        public static async Task SeedProjectTasks(this IApplicationBuilder app)
        {
            ...
            if (!context.ProjectTasks.Any())
            {
                ...
                {
                    ProjectTask.New(ProjectTaskId.New(), projects[0].Id, "Frontend Development", 240, "Develop the website frontend"),
                    ProjectTask.New(ProjectTaskId.New(), projects[0].Id, "Backend Integration", 180, "Integrate backend APIs"),
        ...
        public static async Task SeedProjectUsers(this IApplicationBuilder app)
        {
            ...
            if (!context.ProjectUsers.Any())
            {
                ...
                if (!projects.Any() || !users.Any() || projectManagerRole == null || projectSponsorRole == null || teamMemberRole == null)
                {
        ...
        public static async Task SeedTimeEntries(this IApplicationBuilder app)
        {
            ...
            if (!context.TimeEntries.Any())
            {
                ...
                {
                    throw new Exception("Projects, project tasks, and users must be seeded before adding time entries.");
                }
                ...
                var timeEntries = new[]
                {
                    TimeEntry.New(TimeEntryId.New(), "Coding frontend", DateTime.UtcNow.AddHours(-2), DateTime.UtcNow.AddHours(-1), 60, adminUser.Id, projects[0].Id, projectTasks[0].Id),
                    TimeEntry.New(TimeEntryId.New(), "Testing backend", DateTime.UtcNow.AddHours(-4), DateTime.UtcNow.AddHours(-3), 60, usualUser.Id, projects[0].Id, projectTasks[1].Id),
                    TimeEntry.New(TimeEntryId.New(), "UI redesign", DateTime.UtcNow.AddHours(-6), DateTime.UtcNow.AddHours(-5), 60, usualUser.Id, projects[1].Id, projectTasks[2].Id)
                };
