...
public interface IProjectTaskQueries
{
    ...
    Task<(IReadOnlyList<ProjectTask> ProjectTasks, int TotalCount)> GetAllPaginated(int page, int pageSize, CancellationToken cancellationToken);
    Task<(IReadOnlyList<ProjectTask> ProjectTasks, int TotalCount)> GetAllByUserIdPaginated(Guid userId, int page, int pageSize, CancellationToken cancellationToken);
